# 差距分析：Threadflow 定稿 vs KnowFlow 现状

> 生成日期：2026-09-07
> 上游规范：[THREADFLOW_SPEC.md](THREADFLOW_SPEC.md)（权威，冲突以此为准）
...
| 编号 | 决策 | 内容 |
| --- | --- | --- |
| D1 | 产品名 | 全面启用 **Threadflow**。应用标题/文案/ARB 全部替换“知序/KnowFlow”；`pubspec.name` 改为 `threadflow`；工作目录 `class-productivity/` 暂不重命名（无 git 历史价值，改名留到发布前，避免本轮文档/脚本路径全乱）。 |
...
| D10 | 算法常量显式化 | 定稿公式未给全部数值参数。所有缺口常量集中为默认值（§4 常量表），代码统一读取 `ThreadflowDefaults`，UI 不直接硬编码；常量本身可调（后续“排序规则深度自定义”的载体）。 |
...
> 定稿未给的数值参数统一收口于此，进入 `ThreadflowDefaults`。凡 UI/文档引用同一来源。
